...
function BaseFooterReplyChat<T>(
  ...
  const normalizedExtensions = formatExtensions(
    authorizedExtensions?.split('|'),
  );
      ...
      {attachedImages.length > 0 && (
        <View style={styles.previewContainer}>
          <ListImageSelected
            ...
            imageUrls={attachedImages.map(
              (img) =>
                localImageMap[img.url] || resolveUploadUrl(img.url) || '',
            )}
